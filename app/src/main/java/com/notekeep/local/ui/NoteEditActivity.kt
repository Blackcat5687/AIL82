...
class NoteEditActivity : AppCompatActivity() {
    ...
    private fun scrollCursorAboveKeyboard() {
        binding.contentScroll.post {
            ...
            // >= (not >) so the caret line is scrolled clear the instant it would even touch the
            // keyboard's top edge, never allowed to graze or slide under it even by a pixel.
    ...
    private fun deleteAndFinish() {
        // currentNote loads asynchronously in onCreate, so it can still be null here even for an
        // existing note if delete is tapped right after opening it - before that race, this used
        // to just finish() without deleting anything. noteId itself is set synchronously from the
        // intent, so use that instead.
        val id = noteId
        if (id == -1L) {
            finish()
            return
