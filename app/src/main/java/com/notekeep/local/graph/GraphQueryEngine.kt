...
object GraphQueryEngine {
     ...
     * [edges] is needed so that turning off "الوسوم" hides not just the tag/label hub nodes
     * themselves but every note connected to one of them - the user asked for turning this off to
     * hide the whole cluster (tag/category and every note carrying it), and for turning it back on
     * to bring all of it back, rather than only hiding the hub node while its notes stay visible. */
    fun recomputeFilter(nodes: List<GraphNode>, edges: List<GraphEdge>, filterState: GraphFilterState) {
        val parsed = parseQuery(filterState.query)
        val hiddenHubIds: Set<String> = if (!filterState.showTags) {
            nodes.filter { it.nodeKind == "tag" || it.nodeKind == "label" }.map { it.id }.toSet()
        } else {
            emptySet()
        }
        val notesAttachedToHiddenHubs: Set<String> = if (hiddenHubIds.isEmpty()) {
            emptySet()
        } else {
            edges.mapNotNull { e ->
                when {
                    hiddenHubIds.contains(e.sourceId) -> e.targetId
                    hiddenHubIds.contains(e.targetId) -> e.sourceId
                    else -> null
                }
            }.toSet()
        }
        for (n in nodes) {
            if (hiddenHubIds.contains(n.id) || notesAttachedToHiddenHubs.contains(n.id)) {
                n.visible = false
                continue
            }
