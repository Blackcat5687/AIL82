...
class GraphActivity : AppCompatActivity(), GraphCanvasView.Callbacks {
    ...
    private fun loadGraphData(savedPositions: Map<String, Pair<Float, Float>>) {
        lifecycleScope.launch {
            ...
            // Includes archived notes so they still appear as graph nodes (soft-deleted/trashed
            // notes remain excluded - those are conceptually gone, unlike an archived note which
            // the user has just tucked away from the main list).
            val notes = db.noteDao().getAllExcludingTrashOnce()
